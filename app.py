...
try:
    ...
    COMMON_PASSWORD = st.secrets["APP_PASSWORD"]  # 🔑 Secretsからパスワードを取得
    ...
    st.error(f"【設定エラー】Secretsの読み込みに失敗しました（APP_PASSWORDなどが正しく設定されているか確認してください）。 エラー詳細: {e}")
    st.stop()
...
if not st.session_state.authenticated:
    st.markdown('<div class="main-header"><h1>🔒 認証が必要です</h1><p>Nexus ALT Digital Speaking Test</p></div>', unsafe_allow_html=True)
    st.markdown('<div class="test-card">', unsafe_allow_html=True)
    st.subheader("共通パスワードを入力してください")
    
    entered_password = st.text_input("パスワード", type="password", placeholder="パスワードを入力")
